    ...
    return {
      ...
      Pipe: Pipe = (function() {
        ...
        Pipe.process_tuples = function(pipe_id, in_buffer, in_buffer_length, operation, call, delimiter, terminator) {
          var buffer_flush_size, buffer_padding, entry, field, finalizer, group_field_values, group_fields, i_group_field, idx, in_buffer_fields, is_group_by, num_fields, out_buffer, out_buffer_fields, out_buffer_length, pipe, processor, tuple, tuple_length, writer, _i, _len, _ref, _ref2;
          ...
          for (idx = 0; 0 <= in_buffer_length ? idx < in_buffer_length : idx > in_buffer_length; 0 <= in_buffer_length ? idx++ : idx--) {
            entry = in_buffer[idx];
            if (is_group_by && entry === delimiter) {
              idx += 1;
            ...
            } else if (is_group_by && entry === terminator) {
              finalizer(writer);
            } else {
